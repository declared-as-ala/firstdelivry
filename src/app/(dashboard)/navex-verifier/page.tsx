...
/** Colis Dhay3in — Navex.tn. Mirrors src/app/(dashboard)/verifier against /api/navex-tn/parcels/to-check. */
export default function NavexVerifierPage() {
  const [data, setData] = useState<{ delay: number; count: number; totalCod: number; parcels: NavexTnParcelRow[] } | null>(null)
  const [loading, setLoading] = useState(true)

  const load = useCallback(() => {
    setLoading(true)
    fetch("/api/navex-tn/parcels/to-check").then((r) => r.json()).then((j) => setData(j.data)).finally(() => setLoading(false))
  }, [])
  useEffect(() => { load() }, [load])

  function copyCodes() {
    const codes = (data?.parcels || []).map((p) => p.trackingCode).join("\n")
    navigator.clipboard.writeText(codes).then(() => toast.success(`${data?.parcels.length} codes copiés`))
  }
  function downloadExcel() {
    const rows = [["Code de suivi", "COD", "État Navex", "Date remise", "Jours en cours"]]
    for (const p of data?.parcels || []) rows.push([p.trackingCode, String(p.codAmount ?? ""), p.navexRawEtat || "", frDate(p.handedToNavexAt), String(daysSince(p.handedToNavexAt))])
    const csv = rows.map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n")
    const url = URL.createObjectURL(new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" }))
    const a = document.createElement("a")
    a.href = url; a.download = `colis-navex-a-verifier-${new Date().toISOString().slice(0, 10)}.csv`; a.click()
    URL.revokeObjectURL(url)
  }

  return (
    <div>
      <PageHeader title="Colis Dhay3in Navex"
        subtitle={data ? `En cours depuis plus de ${data.delay} jours — à réclamer à Navex.tn / livreur` : ""}
        action={
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-400">
                ...
                <th className="px-3 py-2.5 font-medium">Date remise à Navex.tn</th>
              ...
              {loading ? (
                ...
                return (
                  <tr key={p._id} className="hover:bg-slate-50">
                    <td className="px-3 py-2.5 font-mono text-xs text-slate-700">{p.trackingCode}</td>
                    <td className="px-3 py-2.5 text-right tabular-nums text-slate-800">{formatTND(p.codAmount)}</td>
                    <td className="px-3 py-2.5 text-slate-600 max-w-[200px] truncate">{p.navexRawEtat || "—"}</td>
