...
import { formatFirstDeliveryStatus } from "@/lib/navex/navex-status.mapper"
...
export default function VerifierPage() {
  ...
  function downloadExcel() {
    const rows = [["Code First Delivery", "COD", "État First Delivery", "Designation", "Date remise", "Jours en cours"]]
    for (const p of data?.parcels || []) rows.push([p.navexTrackingCode, String(p.codAmount), formatFirstDeliveryStatus(p.navexRawStatus), p.designation || "", frDate(p.handedToNavexAt), String(daysSince(p.handedToNavexAt))])
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-400">
                ...
                <th className="px-3 py-2.5 font-medium">État First Delivery</th>
              ...
              {loading ? (
                ...
                return (
                  <tr key={p._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2.5 text-slate-600 max-w-[200px] truncate">{formatFirstDeliveryStatus(p.navexRawStatus)}</td>
