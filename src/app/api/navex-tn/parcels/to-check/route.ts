...
/**
 * "Colis Dhay3in" for Navex.tn — mirrors /api/parcels/to-check. EN_COURS parcels
 * handed over more than `delay` days ago (not paid, not returned).
 */
export async function GET() {
  ...
  const parcels = await NavexTnParcel.find(filter)
    .select("trackingCode codAmount navexRawEtat handedToNavexAt")
